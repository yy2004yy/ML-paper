...
def k_fold(k, X_train, y_train, num_epochs, learning_rate, weight_decay, batch_size):
    ...
    for i, (train_idx, valid_idx) in enumerate(kf.split(X_train)):
        ...
        model = LSTMModel(input_dim=X_train.shape[1], hidden_dim=64, output_dim=1, num_layers=2).to(device)
...
# 训练最终模型
final_model = LSTMModel(input_dim=train_features.shape[1], hidden_dim=64, output_dim=1, num_layers=2).to(device)
