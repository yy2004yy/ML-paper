...
# 划分测试集 (假设20%用于测试集)
test_split = int(0.8 * n_train)
train_features, test_features = train_features[:test_split], train_features[test_split:]
train_labels, test_labels = train_labels[:test_split], train_labels[test_split:]
...
def k_fold(k, X_train, y_train, num_epochs, learning_rate, weight_decay, batch_size):
    ...
    for i, (train_idx, valid_idx) in enumerate(kf.split(X_train)):
        ...
        valid_dataset = TensorDataset(torch.tensor(X_val, dtype=torch.float32), torch.tensor(y_val, dtype=torch.float32))
        ...
        for epoch in range(num_epochs):
            ...
            print(f'Fold [{i + 1}/{k}], Epoch [{epoch + 1}/{num_epochs}], Train Loss: {train_loss:.4f}, Valid log RMSE: {valid_loss:.4f}')

        all_train_loss.append(train_losses)
        all_valid_loss.append(valid_losses)
    ...
    return avg_train_log_rmse, avg_valid_log_rmse, all_train_loss, all_valid_loss

# 设置参数
k, num_epochs, lr, weight_decay, batch_size = 3, 200, 0.0005, 0.0005, 64

# 进行 K-fold 交叉验证
train_l, valid_l, all_train_loss, all_valid_loss = k_fold(k, train_features.cpu().numpy(), train_labels.cpu().numpy(), num_epochs, lr, weight_decay, batch_size)
...
train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
num_epochs_final = 200  # 根据需要设置最终训练的轮数
...
final_model.eval()
test_loss, true_labels, test_preds = Test_model(final_model, test_loader, criterion, device)
print(f'Final Test log RMSE: {test_loss:.4f}')

# 绘制 Train Loss 和 Valid log RMSE 随着epoch的曲线
plt.figure(figsize=(12, 6))
epochs = range(1, num_epochs + 1)
for i in range(k):
    plt.plot(epochs, all_train_loss[i], label=f'Train Loss Fold {i+1}')
    plt.plot(epochs, all_valid_loss[i], label=f'Valid log RMSE Fold {i+1}')

plt.title('Train Loss and Valid log RMSE vs Epochs')
plt.xlabel('Epochs')
plt.ylabel('Loss / log RMSE')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
